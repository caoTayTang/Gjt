...
if [[ $? -eq 0 && -d ".bku" && -f ".bku/history.log" && -f ".bku/tracked_files" ]]; then
...
if [[ $? -eq 0 && $(grep -c "src/main.c" .bku/tracked_files) -eq 1 && -f ".bku/src_main.c/latest" ]]; then
    print_result 0 "Added single file successfully"
else
    print_result 1 "Failed to add single file (check add_single_output.txt)"
fi

# Test 4: Add Files (All Files)
echo "Test 4: Add All Files"
bku add > $RUN_PATH/add_all_output.txt 2>&1
if [[ $? -eq 0 && $(grep -c "src/main.c" .bku/tracked_files) -eq 1 && $(grep -c "src/utils.c" .bku/tracked_files) -eq 1 ]]; then
...
bku status src/main.c > $RUN_PATH/status_single_output.txt 2>&1 echo $?
...
if [[ $? -eq 0 && $(grep -c "Committed src/main.c" $RUN_PATH/commit_single_output.txt) -eq 1 && $(grep -c "Added modification" .bku/history.log) -gt 0 ]]; then
...
# rm -rf *.txt
cleanup
# Do not remove output files immediately to allow inspection
